<template>
  <Modal v-model="addApply"  title="子菜单" :mask-closable="false" @on-ok="ok"
        @on-cancel="cancel">
      <iv-form ref="formValidate" :model="menu" :rules="ruleValidate">
          <iv-form-item v-if=nameFlag label="上级菜单:">
                 {{menu.parentName}}
          </iv-form-item>
...
export default {
  ...
  methods: {
    init (parentParam) {
      this.menu.parentId = parentParam.id
      this.menu.parentName = parentParam.name
      this.menu.type = parentParam.type
      this.nameFlag = true
    },
    editInit (param) {
